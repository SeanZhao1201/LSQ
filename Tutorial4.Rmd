...
```{r}
# First, check the range of energy consumption values
range(dat$energy.kWh)

# Create frequency distribution for annual energy consumption
table(cut(
  dat$energy.kWh,
  breaks = seq(0, 40000, 2000)  # Based on the typical range of energy values
))

# Create histogram for annual energy consumption
dat %>%
  ggplot(mapping = aes(x = energy.kWh)) +
  geom_histogram(
    breaks = seq(0, 40000, 2000)
  ) +
  xlab("Annual Energy Consumption (kWh)")
```
...
```{r}
# Boxplot 1: Overall variability in energy use
dat %>%
  ggplot(mapping = aes(x = energy.kWh)) +
  geom_boxplot() +
  xlab("Annual Energy Consumption (kWh/year)") +
  theme(
    axis.title.y = element_blank(),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank()
...
# Boxplot 2: Comparing energy use between "Near" and "Far" households
dat %>%
  ggplot(mapping = aes(x = energy.kWh, y = distance.ord)) +
  geom_boxplot() +
  xlab("Annual Energy Consumption (kWh/year)") +
  ylab("Distance from city core (below/above Q2)")
```
