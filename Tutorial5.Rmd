...
fishDat %>%
  filter(!is.na(LogViralLoad)) %>%
  ggplot(mapping = aes(
    x = jitter(Dose, factor = 0.5),  # Increased jitter factor
    y = LogViralLoad,
    col = Strain,       # Color-coded by viral strain
    pch = RunType       # Shape-coded by run type (Spring/Stream vs Fall/Ocean)
  )) +
  geom_point(size = 2) +
  xlab("Log Viral Exposure Dose") +
  ylab("Log Viral Load") +
  labs(color = "Viral Strain", shape = "Run Type")
```
...
```{r}
# Fit separate models for each level of RunType (blocking variable)
modSpringStream <- fishDat %>%
  filter(RunType == "Spring/Stream") %>%
  lm(formula = LogViralLoad ~ Dose)

modFallOcean <- fishDat %>%
  filter(RunType == "Fall/Ocean") %>%
  lm(formula = LogViralLoad ~ Dose)

# Display model coefficients
coef(modSpringStream)
coef(modFallOcean)

# Create scatterplot with color-coding by RunType and trendlines
fishDat %>%
  filter(!is.na(LogViralLoad)) %>%
  ggplot(mapping = aes(
    x = jitter(Dose, factor = 0.25),
  ...
  ylab("Log Viral Load") +
  geom_abline(
    intercept = coef(modFallOcean)[1],
    slope = coef(modFallOcean)[2],
    col = 2  # Color for Fall/Ocean
  ) +
  geom_abline(
    intercept = coef(modSpringStream)[1],
    slope = coef(modSpringStream)[2],
    col = 3  # Color for Spring/Stream
  ) +
  labs(color = "Run Type")
```
